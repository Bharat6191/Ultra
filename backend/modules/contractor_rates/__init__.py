...
The package exposes:
  * ``models``        - SQLAlchemy ORM rows (rate_master, contractor_rates,
                        negotiation_logs, contractor_rate_audit_logs)
  ...
  * ``module_config`` - RBAC catalog for ``rate_master`` and ``contractor_rates``
"""
